...
from misc.lib.base.extract import  extract, extract_map
from misc.lib.base.b64uuid import b64uuid
from misc.lib.base.video_link import swf_url_parse, _HTM_SWF
from misc.lib.base.txt import cnenoverflow
...
def txt_withlink(s):
    ...
    s = extract_map('\r{{{', '\r}}}\r', '\r%s\r'%s, replace_code).strip()
...
def txtoverflow(txt, length):
    txt = _link_img(txt)
    txt = RE_IMG.sub('', txt)
    txt = RE_LINK_TARGET.sub('', txt)
    return cnenoverflow(txt, length)
